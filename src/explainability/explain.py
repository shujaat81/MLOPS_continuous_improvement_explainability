...
def explain_with_shap(model, X_background, X_explain, class_names=None, n_samples=100, 
                     output_file=None):
    """
    ...
    if isinstance(shap_values, list):
        ...
        shap.summary_plot(shap_values[0], explain_sample, feature_names=[f"pixel_{i}" for i in range(explain_sample.shape[1])],
                         show=False)
...
def explain_with_lime(model, X_train, X_explain, y_explain, class_names, n_samples, output_file):
    """
    ...
    def predict_fn(x):
        ...
        x_flat = x_gray.reshape(x_gray.shape[0], 28 * 28)  # Flatten to match model input
