...
def apply_pca(x_train, x_test, n_components=100, random_state=42):
    ...
    print(
        f"PCA with {n_components} components explains {explained_variance:.2%} of variance"
    )
...
def apply_tsne(
    x_train, x_test=None, n_components=2, perplexity=30, n_iter=1000, random_state=42
):
    """
...
def build_autoencoder(input_shape, architecture):
    try:
        ...
        for i in range(1, len(architecture) // 2 + 1):
            x = Dense(
                architecture[i], activation="relu", kernel_initializer="he_normal"
            )(x)
...
if __name__ == "__main__":
    ...
    if config["feature_extraction"]["pca"]["apply"]:
        ...
        save_features(
            ...
            output_dir=os.path.dirname(
                config["feature_extraction"]["pca"]["output_file"]
            ),
        )
        save_features(
            x_test_pca,
    ...
    if config["feature_extraction"]["tsne"]["apply"]:
        ...
        save_features(
            ...
            "x_train_tsne.npy",
            output_dir=os.path.dirname(
                config["feature_extraction"]["tsne"]["output_file"]
            ),
        )
    ...
    if config["feature_extraction"]["autoencoder"]["apply"]:
        autoencoder, encoder, history = train_autoencoder(
            ...
            validation_split=config["feature_extraction"]["autoencoder"][
                "validation_split"
            ],
        )
        ...
        save_features(
            ...
            output_dir=os.path.dirname(
                config["feature_extraction"]["autoencoder"]["output_file"]
            ),
        )
        save_features(
            x_test_encoded,
